...
export function getBasePrice(curveLpTokenAddress: Address, curveRegistry: CurveRegistryContract, network: string): CustomPriceType {
...
export function getUnderlyingCoinFromPool(poolAddress: Address, curveRegistry: CurveRegistryContract, network: string): Address {
...
export function getVirtualPrice(curveLpTokenAddress: Address): BigDecimal {
  ...
  let virtualPrice = utils.readValue<BigInt>(curveRegistry.try_get_virtual_price_from_lp_token(curveLpTokenAddress), constants.BIGINT_ZERO).toBigDecimal();
