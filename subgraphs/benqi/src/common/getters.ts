...
export function getOrCreateMarket(event: ethereum.Event, marketAddress: Address): Market {
  ...
  if (!market) {
    ...
    let tryReserveFactor = qiTokenContract.try_reserveFactorMantissa();
...
export function getOrCreateQiToken(tokenAddress: Address, qiContract: BenqiTokenqi): Token {
  ...
  if (cToken == null) {
    ...
    cToken.name = qiContract.name();
    cToken.symbol = qiContract.symbol();
    cToken.decimals = qiContract.decimals();
    cToken.save();
  }
  return cToken;
}
